...
document.addEventListener('keydown', function(event) {
        ...
        if (activePage.id === 'page15-3player-summary') {
            if (event.key === 'Tab') {
                event.preventDefault();
                const backBtn = document.querySelector('#page15-3player-summary .summary3p-back-btn');
        ...
        if (activePage.id === 'page4') {
            ...
            // ✅ NIEUW: Tab is nu CONTEXT-AFHANKELIJK — kijkt wat er op dat
            // moment gemarkeerd is, en handelt daarnaar (start de match ENKEL
            // als een bal-optie gemarkeerd is, klik anders gewoon de knop).
            if (event.key === 'Tab') {
                event.preventDefault();
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // ✅ Bewaar globaal beschikbaar, voor het geval elders nog naar
    // window.highlightPage1Button/clearPage1Dim verwezen wordt.
...
window.showDamesSpelersKeuze = async function() {
    const res = await fetch('https://kpbcdemo.pythonanywhere.com/api/dames/spelers-en-paren');
