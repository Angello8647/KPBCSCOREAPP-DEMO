...
window.loadFilteredMatches = function() {
    ...
    let html = `<div class="matches-list-title" style="font-size: 2.0rem; margin-bottom: 30px; font-weight: bold; color: #ffffff; text-align: center;">${filtered.length} matchen voor ${dayName} ${dateStr}</div>`;
    ...
    sortedKeys.forEach(key => {
        ...
        html += `<h3 style="color: #f1c40f; margin: 0 0 25px 0; font-size: 2.0rem; border-bottom: 2px solid rgba(255,255,255,0.1); padding-bottom: 12px; text-align: center;">
                    ⚪🟡🔴 ${group.displayTitle} <span style="color: #95a5a6; font-size: 0.7em;">(${group.matches.length} matchen)</span>
                 </h3>`;
        
        html += `<div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(320px, 1fr)); gap: 35px;">`;
